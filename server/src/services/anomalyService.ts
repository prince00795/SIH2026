export interface IMarketAnomaly {
  anomalyId: string;
  type: 'HORIZON_INVERSION' | 'CORRIDOR_DIVERGENCE' | 'PRICE_SPIKE' | 'PRICE_DROP';
...
export class AnomalyService {
  public static scanAnomalies(
    routeObservations: Array<{ routeCode: string; advanceWindow: string; meanFare: number }>,
    topRoutes: any[]
  ): IMarketAnomaly[] {
    const anomalies: IMarketAnomaly[] = [];
    ...
    for (const [routeCode, windows] of routeMap.entries()) {
      ...
      if (fwdT7 && revT7 && Math.min(fwdT7, revT7) > 0) {
        ...
        const minFare = Math.min(fwdT7 - revT7);
