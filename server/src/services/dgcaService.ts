...
import { RouteModel, IRoute } from '../models/Route.js';
import { RouteWeightModel, IRouteWeightVersion, IBasketRouteItem } from '../models/RouteWeight.js';

export const CITY_IATA_MAP: Record<string, { iata: string; city: string }> = {
...
export class DGCAService {
  ...
  public static getProcessedData() {
  ...
  public static getTopRoutes(topN = 20): IBasketRouteItem[] {
    ...
    const basketSum = sliced.reduce((acc: number, item: any) => acc + item.total_two_way_passengers, 0);
    ...
    return sliced.map((item: any, idx: number) => {
      ...
      return {
        ...
        weightLabel: 'DGCA TRAFFIC-DERIVED WEIGHT' as const,
      };
    });
  }

  public static getAllCityPairs(limit = 100) {
    const data = this.getProcessedData();
  ...
  public static getRouteMetadata(routeCode: string) {
