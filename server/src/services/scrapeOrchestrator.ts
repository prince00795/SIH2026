...
import { ScrapeRunModel, IScrapeRun } from '../models/ScrapeRun.js';
...
import { ScraperBase } from '../scrapers/core/ScraperBase.js';
...
const ADAPTER_FACTORIES: Record<string, () => ScraperBase> = {
...
export interface ITriggerScrapeOptions {
  sourceNames?: string[];
  routeCodes?: string[];
  horizons?: Array<'T+1' | 'T+7' | 'T+15' | 'T+30' | 'T+45'>;
  mode?: 'LIVE' | 'DEMO';
}

export class ScrapeOrchestrator {
  private static inMemoryRuns = new Map<string, any>();

  public static async startScrapeRun(options: ITriggerScrapeOptions = {}): Promise<string> {
    const runId = `RUN-${Date.now()}-${Math.random().toString(36).substring(2, 6).toUpperCase()}`;
    const mode = options.mode || (ENV.SCRAPER_MODE as 'LIVE' | 'DEMO');
  ...
  private static async executeRun(runId: string, run: any): Promise<void> {
    run.status = 'RUNNING';

    const horizonDaysMap: Record<string, number> = {
      'T+1': 1,
      'T+7': 7,
      'T+15': 15,
      'T+30': 30,
  ...
  public static async getRunById(runId: string): Promise<any | null> {
  ...
  public static async listRuns(limit = 20): Promise<any[]> {
  ...
  public static async getSourceHealthSummary(): Promise<any[]> {
    ...
    return REQUIRED_SOURCES.map((s) => {
      ...
      const sourceRuns = runs.filter((r) =>
        r.sourceResults?.some((sr: any) => sr.sourceName === s.name)
      );
      ...
      const resInLatest = latestRun.sourceResults?.find((sr: any) => sr.sourceName === s.name);
      ...
      for (const r of sourceRuns) {
        const sr = r.sourceResults?.find((x: any) => x.sourceName === s.name);
        if (!sr) continue;
