...
export class SourceController {
  public static async getSourcesCatalog(req: Request, res: Response): Promise<void> {
    try {
  ...
  public static async getSourceHealth(req: Request, res: Response): Promise<void> {
    try {
      const mode = (req.query.mode as string) === 'LIVE' ? 'LIVE' : 'DEMO';

      if (mode === 'LIVE') {
        // Enforce Phase 12: Source Health must be calculated from actual ScrapeRun records
  ...
  public static async triggerSourceScrape(req: Request, res: Response): Promise<void> {
    try {
