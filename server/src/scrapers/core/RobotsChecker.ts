...
export type RobotsComplianceStatus = 'ALLOWED' | 'DISALLOWED' | 'ROBOTS_UNVERIFIED';

export class RobotsChecker {
  private static cache = new Map<string, { rules: string[]; status: RobotsComplianceStatus; timestamp: number }>();
  private static CACHE_TTL_MS = 24 * 60 * 60 * 1000; // 24 hours

  public static async checkPermission(
    targetUrl: string,
    userAgent = 'VayuSutra-APIx'
  ): Promise<{ status: RobotsComplianceStatus; isAllowed: boolean; reason: string }> {
    try {
      ...
      const status: RobotsComplianceStatus = isAllowed ? 'ALLOWED' : 'DISALLOWED';
  ...
  public static async isAllowed(targetUrl: string): Promise<boolean> {
    const res = await this.checkPermission(targetUrl);
    return res.isAllowed;
  }

  private static fetchRobotsTxt(urlStr: string): Promise<{ status: 'OK' | 'ERROR'; content: string }> {
  ...
  private static evaluateRules(rules: string[], path: string): boolean {
    ...
    for (const line of rules) {
      ...
      if (lower.startsWith('user-agent:')) {
        ...
        appliesToBot = ua === '*' || ua.toLowerCase().includes('vayusutra');
